...
FINP_GLOB="${STM_FINPUT_GLOB:-*.finp}"
SUMMARY_FILE="${STM_SUMMARY_FILE:-${WORK_DIR}/gpu_batch_summary.tsv}"
...
for finp in ${FINP_GLOB}; do
    ...
    scan_seconds="$(grep -oP '(?<=Scan completed in )[^s]+' "${out_log}" | tail -n 1 | tr -d '[:space:]')"
    total_seconds="$(grep -oP '(?<=Total time: )[^s]+' "${out_log}" | tail -n 1 | tr -d '[:space:]')"
    matrix_name="$(printf '%s\n' "${plot_output}" | awk -F': ' '/^Matrix:/ {print $2}' | tail -n 1)"

    printf "%s	%s	%s	%s	%s	%s\n"         "${finp}"         "${scan_seconds}"         "${total_seconds}"         "${matrix_name}"         "${out_m}"         "${out_png}" >> "${SUMMARY_FILE}"
done
